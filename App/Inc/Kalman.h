...
typedef struct{
	...
	float InOut;
  	...
  	Kalman_Structure Vac_R_Klmn,Vac_Y_Klmn,Vac_B_Klmn,Iac_R_Klmn,Iac_Y_Klmn,Iac_B_Klmn; // kalman filter variables
    float *Kalman_Filter(float*, float, float, float, float, float);
    void Kalman_Filter1(Kalman_Structure *k_temp, float Sample_Freq, float Wn, float Q1, float Q2, float Q3, float Q4, float R1);

#else
    extern float *Kalman_Filter(float*, float, float, float, float, float);
    extern void Kalman_Filter1(Kalman_Structure *k_temp, float Sample_Freq, float Wn, float Q1, float Q2, float Q3, float Q4, float R1);
    extern Kalman_Structure *PtrKalman;
    extern Kalman_Structure Vac_R_Klmn,Vac_Y_Klmn,Vac_B_Klmn,Iac_R_Klmn,Iac_Y_Klmn,Iac_B_Klmn; // kalman filter variables
#endif
